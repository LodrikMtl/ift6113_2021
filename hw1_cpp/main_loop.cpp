...
double weighted_valence(double n) {
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < prevNumVertices; i++) {
        ...
        int n = neighs.size();
        double w_n = weighted_valence(n);
        V.row(i) = w_n * prevV.row(i);
        std::cout << V.row(i) << " VS " << prevV.row(i) << "\n";
        for (auto neigh : neighs) {
            V.row(i) += prevV.row(neigh);
        }
    ...
    for (int i = 0; i < prevEdges.size(); i++) {
        ...
        double stencil[] = { 3, 0, 1, 0, 3, 0, 1 , 0 };
    ...
    viewer.data().add_points(prevV, Eigen::RowVector3d(1, 0, 0));
